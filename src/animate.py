...
def PhotoPanUp(img, d):
	...
	# RENDER
	# cmd = FFMPEG_BIN + " -y -loop 1 -loglevel panic -i "+img+" \
	# -c:v libx264 -pix_fmt yuv420p \
	# -filter_complex \
	# \"[0:v]crop=h=ih:w='if(gt(a,16/9),ih*16/9,iw)':y=0:x='if(gt(a,16/9),(ow-iw)/2,0)'[v01]; \
	# [v01]scale=-1:4000,crop=w=iw:h=iw*9/16:x=0:y='"+start_y_percent+"*ih-((t/"+d+")*min("+start_y_percent+"*ih,(ih-oh)/6))',trim=duration="+d+"[v02]; \
	# [v02]zoompan=z='if(lte(pzoom,1.0),1.15,max(1.0,pzoom-0.0005))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1,setsar=sar=1:1[v]\" \
	# -map \"[v]\" -s \"1280x720\" " + video_path

	cmd = FFMPEG_BIN + " -y -loop 1 -loglevel panic -i %s \
	-c:v libx264 -pix_fmt yuv420p \
	-filter_complex \
	\"[0:v]crop=h=ih:w='if(gt(a,16/9),ih*16/9,iw)':y=0:x='if(gt(a,16/9),(ow-iw)/2,0)'[v01]; \
	[v01]scale=-1:4000,crop=w=iw:h=iw*9/16:x=0:y='%.2f*ih-((t/%.2f)*min(%.2f*ih,(ih-oh)/6))',trim=duration=%.2f[v02]; \
	[v02]zoompan=z='if(lte(pzoom,1.0),1.15,max(1.0,pzoom-0.0005))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1,setsar=sar=1:1[v]\" \
...
def PhotoPanDown(img, d):
	...
	# -filter_complex \
	# \"[0:v]crop=h=ih:w='if(gt(a,16/9),ih*16/9,iw)':y=0:x='if(gt(a,16/9),(ow-iw)/2,0)'[v01]; \
	# [v01]scale=-1:4000,crop=w=iw:h=iw*9/16:x=0:y='max((ih-oh)/6,"+start_y_percent+"*ih-((ih-oh)/6))+((t/"+d+")*(ih-oh)/6)',trim=duration="+d+"[v02]; \
	...
	-filter_complex \
	\"[0:v]crop=h=ih:w='if(gt(a,16/9),ih*16/9,iw)':y=0:x='if(gt(a,16/9),(ow-iw)/2,0)'[v01]; \
	[v01]scale=-1:4000,crop=w=iw:h=iw*9/16:x=0:y='max((ih-oh)/6,%.2f*ih-((ih-oh)/6))+((t/%.2f)*(ih-oh)/6)',trim=duration=%.2f[v02]; \
	[v02]zoompan=z='min(pzoom+0.0005,1.5)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1,setsar=sar=1:1[v]\" \
