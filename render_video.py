...
def handler(event, context):
	...
	cmd = "%s -f concat -safe 0 -i /tmp/concat.txt -c copy -y %s" % (common.FFMPEG_BIN, video_file)
	common.executeCmd(cmd)

	# Get final video duration
	cmd = common.FFMPEG_BIN + " -i /tmp/concat.mp4 2>&1 | grep \"Duration\"| cut -d ' ' -f 4 | sed s/,// | sed 's@\..*@@g' | awk '{ split($1, A, \":\"); split(A[3], B, \".\"); print 3600*A[1] + 60*A[2] + B[1] }'"
